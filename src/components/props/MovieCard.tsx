...
const MovieCard = ({ title, poster_path, release_date, vote_average, overview, genres, runtime, section, id }: MovieTypes) => {
  return (
    <Popover trigger="hover" placement="auto" gutter={10} >
      ...
      <PopoverContent _focus={{ boxShadow: 'none' }} width="300px" background={'transparent'} backdropFilter={'auto'} border={0} backdropBlur={'80px'}>
        <PopoverArrow />
        ...
        <PopoverBody>
          <VStack spacing={3} alignItems={'start'} h={'100%'} justify={'center'} padding={4} color="black">
            ...
            <Flex w="100%" justifyContent="space-between" alignItems="center" fontSize="sm">
              ...
              <Text fontWeight="bold" color="yellow.300">IMDB {vote_average.toString().split(".")[0] + "." + vote_average.toString().split(".")[1]}</Text>
              <Text fontWeight="bold" color="gray.300">{release_date.toString().split('-')[0]}</Text>
              <Text fontWeight="bold" color="gray.300">{runtime}min</Text>
            </Flex>
